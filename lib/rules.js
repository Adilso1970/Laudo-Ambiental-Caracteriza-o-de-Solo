...
export function buildObrigacoes(projeto) {
  ...
  if (indicios.some(x => alvo.includes(x))) {
    push(
      ...
      "O histórico/indícios apontam possibilidade de passivo ambiental. Recomenda-se triagem e, se necessário, investigação por etapas, definindo amostragem e parâmetros compatíveis (solo/água subterrânea).",
      ["CONAMA_420_2009"],
      [],
      "alta"
    );
  ...
  if (interv.captacaoAgua === true || interv.poco === true) {
    push(
      ...
      "Foi indicada captação de água e/ou poço. Avaliar necessidade de outorga/regularização, incluindo enquadramento de águas subterrâneas e requisitos locais do órgão gestor.",
  ...
  if (interv.mataAtlanticaProvavel === true) {
    push(
      "Regras específicas para Mata Atlântica",
      "Foi indicada provável inserção em área de Mata Atlântica. Aplicam-se regras específicas para supressão/estágio sucessional e autorizações correlatas.",
      ["LEI_11428_2006"],
      [],
      "alta"
    );
